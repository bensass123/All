...
//game variable
var g;
var currentXRect, currentYRect = 320;
  ...
  // $('#dice').html('Player with id: ' + this.id + ' rolled ' + dice1 + " & " + dice2);
	  ...
	  if (this.doubleRolls < 3){
	  	if ((this.location + total) < 40) {
	  		this.location += total;
	  		// proof of concept only, method to compartmentalize piece movement
	  		moveRectangleLeft(50 + 100 * total);
	  	}
	  	else {
	  			//award player 200
	  		this.passedGo();
...
function createInfoCanvas() {
		console.log('data canvas create start');
		var d = $('<canvas/>').attr({
			id: 'dCanvas'
		});
...
function handleStartGameButton(){
	$('#startGame').on('click', function() {
		...
		$('.app').empty();
		$('.app').css('background', 'url(../img/1pixel.jpg) no-repeat center top');
		// $('.app').css('padding', '0px 0px 0px 340px');
		// $('.app').css('margin', '-170px 0px 0px -170px');
		// $('.app').css('left', '25%')
		// $('.app').css('height', '95%');
		// $('.app').css('width', '100%');

		//create canvas
		console.log('canvas create start');
		var c = $('<canvas/>').attr({
			id: 'bCanvas'
		});
		...
		c.css({
			...
			'border-radius': '5px',
			'background': 'url(../img/monoBoardSmall.jpg) no-repeat center top',
			'border-style': 'solid',
    		'border-width': '1px'
		});

		$('.app').addClass('appPlaying');
		$('.appPlaying').removeClass('app');

		$('.appPlaying').append(c);
		console.log('canvas created');
		
		console.log('done w start');
...
// original methods for browser canvas object

function drawRectanglePiece(xRect, yRect){
	var xRect, yRect, xText, yText;
	currentXRect = xRect;
	currentYRect = yRect;
	xText = xRect + 6;
	yText = yRect + 39;
	...
	if (canvas.getContext) {
	    ...
	    ctx.fillRect(xRect, yRect, 300, 300);
...
// drawRectanglePiece(1130,1130);



// function moveRectangleLeft(dist){
// 	drawRectanglePiece(currentXRect - dist, currentYRect);
// }
